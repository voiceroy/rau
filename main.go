...
func HandleInstall(r GithubRelease, installPath string, httpClient *http.Client) error {
	...
	case "darwin":
		{
			return handleDarwin(r, httpClient)
		}
	case "linux":
		{
			return handleLinux(r, httpClient)
		}
	case "windows":
		{
			return handleWindows(r, httpClient)
		}
	}

	return fmt.Errorf("operating sytem %s is not supported yet", OS)
}
...
func handleDarwin(r GithubRelease, httpClient *http.Client) error {
	panic("implement")
}

func handleLinux(r GithubRelease, httpClient *http.Client) error {
	ARCH := getArch()
	...
	if err := uncompressGZ(tempFile, extractedFile); err != nil {
		return err
	}
...
func handleWindows(r GithubRelease, httpClient *http.Client) error {
	panic("implement")
}
...
func uncompressGZ(file *os.File, destinationFile *os.File) error {
